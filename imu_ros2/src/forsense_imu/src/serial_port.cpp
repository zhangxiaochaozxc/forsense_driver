...
class IMUPublisher : public rclcpp::Node
{
	public:
		...
		IMUPublisher() : Node("IMU_publisher")	
		{
            ...
            Send_CMD_LONG(3,1,0,0,0,0,0);
			for(int i = 0; i < 10; i++)
			write(fd, (uint8_t *)&data_cmd_long, sizeof(data_cmd_long));
	...
	private: 
		...
		// void imu_write(const uint8_t *data, size_t size)
		// {
		// 	if (data == NULL || size == 0) {
		// 	// 没有数据需要写入，或者提供的数据大小为0
		// 	return;
		// 	}
		...
		// 	n_written = write(fd, data + total_written, size - total_written);
		// 	if (n_written < 0) {
		// 	break;
		// 	}
